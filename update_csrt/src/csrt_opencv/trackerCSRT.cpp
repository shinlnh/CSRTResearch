...
public:
    ...
    // ========== EXPOSED API FOR DUAL-BRANCH ARCHITECTURE ==========
    /** @brief Get the last computed correlation response map */
    Mat getResponseMap() const { return last_response_map_.clone(); }
    
    /** @brief Get the binary mask used in CSRT (m in h=m⊙h constraint) */
    Mat getMask() const { return filter_mask.clone(); }
    
    /** @brief Get filter coefficients in frequency domain */
    std::vector<Mat> getFilterFrequency() const { return csr_filter; }
    
    /** @brief Get filter coefficients in spatial domain (for analysis) */
    std::vector<Mat> getFilterSpatial() const;
    
    /** @brief Get current object center */
    Point2f getObjectCenter() const { return object_center; }
    
    /** @brief Get current scale factor */
    float getScaleFactor() const { return current_scale_factor; }
    
    /** @brief Get window function used in CSRT */
    Mat getWindow() const { return window.clone(); }
    
    /** @brief Get cell size (HOG cell size) */
...
protected:
    ...
    // Cache for last response map (for dual-branch architecture)
    Mat last_response_map_;
};
...
Point2f TrackerCSRTImpl::estimate_new_position(const Mat &image)
{
    ...
    // Cache response map for dual-branch architecture
    last_response_map_ = resp.clone();
...
std::vector<Mat> TrackerCSRTImpl::getFilterSpatial() const
{
    std::vector<Mat> spatial_filters;
    spatial_filters.reserve(csr_filter.size());
    
    for (size_t i = 0; i < csr_filter.size(); ++i) {
        Mat spatial;
        // Convert from frequency domain to spatial domain
        idft(csr_filter[i], spatial, DFT_SCALE | DFT_REAL_OUTPUT);
        spatial_filters.push_back(spatial);
    }

...
def train_one_epoch(
    ...
    for batch_idx, batch in enumerate(pbar):
        ...
        for b in range(B):
            ...
            h_csrt_list.append(h_csrt_b.unsqueeze(0))  # Add batch dim: (1, C, H, W)
        h_csrt = torch.cat(h_csrt_list, dim=0)  # (B, C, H, W) = (B, 31, H, W)
        ...
        for b in range(B):
            response_b = model.dcf_solver.apply_filter(
                h_proj[b:b+1], h_final[b]  # h_final[b] shape: (C, H, W)
            )  # Returns (1, H, W)
            response[b] = response_b.squeeze(0)  # Remove batch dim
